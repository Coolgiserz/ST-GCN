...
def load_pems_m_data():
    ...
    if (not os.path.isfile("data/PeMS-M/W_228.csv")
            or not os.path.isfile("data/PeMS-M/V_228.csv")):
...
def get_normalized_adj(A):
    ...
    """
    A = A + np.diag(np.ones(A.shape[0], dtype=np.float32)) # +1平滑，自连接
    D = np.array(np.sum(A, axis=1)).reshape((-1,)) # 度矩阵
    D[D <= 10e-5] = 10e-5    # Prevent infs
    diag = np.reciprocal(np.sqrt(D))  # 求逆，元素级别
    A_wave = np.multiply(np.multiply(diag.reshape((-1, 1)), A),
                         diag.reshape((1, -1))) # 拉普拉斯矩阵
    return A_wave
